...
//			driver = new FirefoxDriver();
		...
		public void testLoginWithCorrectCredentials() {
			...
			Assert.assertEquals(homepage.isLoggedInUsernameVisible(), "vijaynk"); 
			homepage.clickDeleteAccoutBtn();
			Assert.assertEquals(homepage.isAccountDeletedVisible(), "ACCOUNT DELETED!");
		}
		...
		public void testLoginWithIncorrectCredentials(){
			...
			loginpage.clickOnLoginBtn();
			Assert.assertEquals(loginpage.error_Message(), "Your email or password is incorrect!"); // Verify error message 'Your email or password is incorrect!' is visible
		}
		...
		public void testRegisterUserWithExistingEmail()
		{
			...
			Assert.assertEquals(homepage.emailAlreadyExist(), "Email Address already exist!"); //verify error 'Email Address already exist!' is visible
		}
...
//	        	//Assert.assertEquals(product.getText().contains("top"), searchpage.getProductName().contains("top"), "products related to the search are visible");
//	            Assert.assertEquals( searchpage.getProductName(),searchpage.spresult(), "products related to the search are visible");
//	        }

//			String expectedProductName = productspage.getProductName(); 
//		    String expectedProductCategory = productspage.getProductCategory();
//		    String expectedProductPrice = productspage.getProductPrice(); 
//		    String expectedProductCondition = productspage.getProductCondition();
//		    String expectedProductBrand = productspage.getProductBrand();
	//
...
//	        Assert.assertTrue(productspage.getProductCategory(), "category not displayed");
//	        Assert.assertTrue(productspage.getProductPrice(), "price not displayed");
//	        Assert.assertTrue(productspage.isProductAvailable(), "availability not displayed");
//	        Assert.assertTrue(productspage.getProductBrand(), "brand not displayed");
//	        Assert.assertTrue(productspage.getProductName(), "name not displayed");
//	        Assert.assertTrue(productspage.getProductCondition(), "condition not displayed");
		...
		public void testVerifyProductQuantityInCart() throws InterruptedException, AWTException
		{
			...
			Assert.assertTrue(productspage.productInfoDetails(), "Product Details Not Visible");
			//Assert.assertEquals(homepage.viewproductNameHomepage(),"Summer White Top");
			productspage.increaseQuantity(4);
			productspage.addToCartBtnInProductInfo();
			productspage.clickOnviewCartlink();
			String actualQuanty = cartpage.getProductQuantity();
			 Assert.assertEquals(actualQuanty, "4");
		...
		public void testPlaceOrder_RegisterWhileCheckout() throws InterruptedException, AWTException
		{
	        ...
	        homepage.newUserSignup("vijaynagendrk", "vijaynagendrk@gmail.com");
			Assert.assertEquals(homepage.isEnterAccountInfoVisible(), "ENTER ACCOUNT INFORMATION");
			homepage.enterAccountInformation("koona2019", "minnu", "Sri", "Automation Project", "Gayatri Nagar",
					"Road no.6", "Telanga", "Hyderabad", "500079", "9000933214");
			Assert.assertEquals(homepage.isAccountCreatedVisible(), "ACCOUNT CREATED!");
			homepage.clickOnContinueBtn();
			Assert.assertEquals(homepage.isLoggedInUsernameVisible(), "vijaynagendrk");
			subscribe.clickOnCartButton();
			checkoutpage.clickOnProceedToCheckout();
		...
		{
			 homepage.clickOnsignup_login();
		        homepage.newUserSignup("vijaynagendrak", "vijayknagendra@gmail.com");
				Assert.assertEquals(homepage.isEnterAccountInfoVisible(), "ENTER ACCOUNT INFORMATION");
				homepage.enterAccountInformation("koona2019", "minnu", "Sri", "Automation Project", "Gayatri Nagar",
						"Road no.6", "Telanga", "Hyderabad", "500079", "9000933214");
				Assert.assertEquals(homepage.isAccountCreatedVisible(), "ACCOUNT CREATED!");
				homepage.clickOnContinueBtn();
				Assert.assertEquals(homepage.isLoggedInUsernameVisible(), "vijaynagendrak");
				productspage.hoverOverFirstProduct();
				productspage.clickcOnCtinueShoppingBtn();
				productspage.hoverOverSecondProduct();
				productspage.clickOnviewCartlink();
